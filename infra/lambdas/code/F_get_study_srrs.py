    ...
    if event:
        ...
        for record in event['Records']:
            try:
                ...
                try:
                    srp = get_srp_sra_project(sra_project_id)
                    ...
                    if srrs:
                        ...
                        store_srrs_in_db(srrs, sra_project_id)
                        ...
                        store_missing_srr_in_db(sra_project_id, PysradbError.NOT_FOUND, 'No SRR found')
                except AttributeError as attribute_error:
                    logging.info(f'For SRP with id {sra_project_id}, pysradb produced attribute error with name {attribute_error.name}')
                    store_missing_srr_in_db(sra_project_id, PysradbError.ATTRIBUTE_ERROR, str(attribute_error))
...
def store_srrs_in_db(srrs: [str], sra_project_id: int):
    try:
...
def get_srp_sra_project(sra_project_id: int) -> str:
    try:
        statement = f'select srp from sra_project where id=%s'
        parameters = (sra_project_id,)
        row = postgres_connection.execute_read_statement(statement, parameters)
        return row[0][0]
    except Exception as exception:
        logging.error(f'An exception has occurred: {str(exception)}')
        raise exception
...
def store_missing_srr_in_db(sra_project_id: int, pysradb_error: PysradbError, details: str):
    try:
        pysradb_error_reference_id = get_pysradb_error_reference(pysradb_error)
        statement = f'insert into sra_run_missing (sra_project_id, pysradb_error_reference_id, details) values (%s, %s, %s);'
...
def get_pysradb_error_reference(pysradb_error: PysradbError) -> int:
    try:
