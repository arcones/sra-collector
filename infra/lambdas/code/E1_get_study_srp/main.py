...
def handler(event, context):
    if event:
        ...
        lambda_log_support.configure_logger(request_id)
        ...
        for record in event['Records']:
            ...
            logging.info(f'Raw pysradb response for {gse} is {raw_pysradb_response}')
            srp = raw_pysradb_response['study_accession'][0]

            if srp:
                logging.info(f"SRP {srp} for GSE {gse} retrieved via pysradb for study {study_with_missing_srp['study_id']}, pushing message to study summaries queue")
